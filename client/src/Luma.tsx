...
interface LumaResponse {
  response: string | SymptomAnalysisResponse;
  type: 'symptom' | 'error' | 'text';
}

const [lumaResponse, setLumaResponse] = useState<LumaResponse | null>(null); // Can be symptom analysis or SRH response
  ...
  const handleAskLuma = async () => {
    ...
    try {
      ...
      if (!token) {
        setLumaResponse({ response: 'Error: Not authenticated. Please log in.', type: 'error' });
        setLoading(false);
        return;
      }

      const response = await fetch(`${process.env.VITE_API_BASE_URL}/api/luma/query`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.type === 'symptom' && (data.response.severity === 'high' || data.response.severity === 'critical')) {
          if (socket) {
        ...
        setLumaResponse({ response: `Error: ${data.message || 'Something went wrong.'}`, type: 'error' });
      }
    } catch (error) {
      setLumaResponse({ response: `Network Error: ${error instanceof Error ? error.message : String(error)}`, type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold mb-8 text-text dark:text-dark-text text-center">Ask Luma</h1>
      <div className="bg-card dark:bg-dark-card p-8 rounded-lg shadow-md border border-border dark:border-dark-border">
        <div className="mb-6">
          <label htmlFor="query" className="block text-lg font-medium text-muted dark:text-dark-muted mb-2">
            What would you like to ask Luma?
          </label>
          <textarea
            id="query"
        ...
        {lumaResponse && (
          <div className="mt-8 p-6 bg-background dark:bg-dark-background rounded-lg border border-border dark:border-dark-border">
            <h3 className="text-xl font-semibold mb-2 text-text dark:text-dark-text">Luma's Response:</h3>
            {lumaResponse.type === 'symptom' ? (
              <div>
                <p className="text-muted dark:text-dark-muted">Primary Diagnosis: {lumaResponse.response.primaryDiagnosis}</p>
                <p className="text-muted dark:text-dark-muted">Severity: {lumaResponse.response.severity}</p>
                <p className="text-muted dark:text-dark-muted">Recommendations: {lumaResponse.response.recommendations.join(', ')}</p>
              </div>
            ) : (
              <p className="text-muted dark:text-dark-muted">{lumaResponse.response}</p>
            )}
          </div>
        )}
      </div>
