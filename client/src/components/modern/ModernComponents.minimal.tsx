...
export const ModernButton: React.FC<ModernButtonProps> = ({
  ...
  const variantClasses = {
    ...
    secondary: 'bg-transparent border border-charcoal-600 text-charcoal-400 hover:bg-charcoal-900 hover:text-charcoal-300 hover:border-charcoal-500',
    ghost: 'text-charcoal-400 hover:text-charcoal-300 hover:bg-charcoal-900/50',
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full ${icon ? 'pl-12' : 'pl-4'} ${type === 'password' ? 'pr-12' : 'pr-4'} py-3 bg-dark-input border border-dark-border rounded-lg text-dark-text placeholder-charcoal-400 focus:outline-none focus:ring-2 focus:ring-charcoal-500/30 focus:border-charcoal-500 transition-all duration-200 ${
        ...
        {type === 'password' && (
          <button
            ...
            className="absolute right-4 top-1/2 transform -translate-y-1/2 text-charcoal-400 hover:text-charcoal-300 transition-colors"
          >
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        <button
          ...
          disabled={disabled}
          className={`w-full px-4 py-3 bg-dark-input border border-dark-border rounded-lg text-left text-dark-text focus:outline-none focus:ring-2 focus:ring-charcoal-500/30 focus:border-charcoal-500 transition-all duration-200 flex items-center justify-between ${
            error ? 'border-danger-500 focus:ring-danger-500/30' : ''
          } ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-dark-surface'}`}
        >
          <span className={selectedOption ? 'text-dark-text' : 'text-charcoal-400'}>
            {selectedOption ? selectedOption.label : placeholder}
          </span>
          <ChevronDown 
            className={`w-5 h-5 text-charcoal-400 transition-transform duration-200 ${
              isOpen ? 'rotate-180' : ''
            }`} 
          />
        </button>
            ...
            >
              {options.map((option) => (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left text-dark-text hover:bg-dark-surface transition-colors first:rounded-t-lg last:rounded-b-lg ${
                    value === option.value ? 'bg-charcoal-600/20 text-charcoal-300' : ''
                  }`}
                >
                  {option.label}
                </button>
...
export const ModernCheckbox: React.FC<ModernCheckboxProps> = ({
  ...
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
      ...
      {label && (
        <label 
          className={`text-dark-text-secondary select-none ${
            disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
          }`}
          onClick={() => !disabled && onChange?.(!checked)}
        >
            ...
            <div className={`relative w-full ${sizeClasses[size]} bg-dark-card backdrop-blur-xl border border-dark-border rounded-xl shadow-2xl max-h-[90vh] overflow-hidden`}>
              ...
              {title && (
                <div className="flex items-center justify-between p-6 border-b border-dark-border">
                  ...
                  <button
                    onClick={onClose}
                    className="text-charcoal-400 hover:text-charcoal-300 transition-colors"
                  >
                    <X size={24} />
                  </button>
                </div>
